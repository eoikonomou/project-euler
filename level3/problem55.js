 ...
 * Due to the theoretical nature of these numbers, and for this problem, we shall assume that a number is Lychrel until proven otherwise.
...
function isLychrel(n) {
    for (let i = 0; i < 50; i++) {
        ...
        if ([...n.toString()].reverse().join('') === n.toString()) return false;
    }
    return true;
}
...
function problem55() {
    ...
    for (let i = 1; i < 10000; i++) {
        if (isLychrel(parseInt(i))) count++;
    }
    return count;
}

console.time('Problem 55');
console.log(problem55());
